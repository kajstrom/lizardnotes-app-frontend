...
void main() {
  ...
  group('ContentPipeline.fromMarkdown', () {
    ...
    test('fromMarkdown strips extra bare newlines after ln-image block',
        () async {
      final delta = Delta()
        ..insert({
          'ln-image': {
      ...
      final markdown = ContentPipeline.toMarkdown(doc);
      final resultDoc = await ContentPipeline.fromMarkdown(markdown);

      final ops = resultDoc.toDelta().toList();
      final imageIndex = ops.indexWhere((op) {
        final d = op.data;
        return d is Map && d.containsKey('ln-image');
      });
      expect(imageIndex, isNot(-1));

      // Count consecutive bare '\n' ops immediately after the image op
      int bareNewlineCount = 0;
      for (int k = imageIndex + 1; k < ops.length; k++) {
        final d = ops[k].data;
      ...
      expect(bareNewlineCount, equals(1),
          reason:
              'Expected exactly one bare newline after ln-image, got $bareNewlineCount');
    });

    test('toMarkdown is stable across two round-trips with an ln-image',
        () async {
      final delta = Delta()
        ..insert({
          'ln-image': {
      ...
      final markdown1 = ContentPipeline.toMarkdown(doc);
      final doc2 = await ContentPipeline.fromMarkdown(markdown1);
      final markdown2 = ContentPipeline.toMarkdown(doc2);

      expect(markdown2, equals(markdown1),
          reason: 'Markdown should be identical across two round-trips');
    });
  });
}
